...
// primary config:
const title = 'Labregister Advanced Item Manager';
...
module.exports = ({ production } = {}, {extractCss, analyze, tests, hmr, port, host, proxy } = {}) => ({
  resolve: {
    extensions: ['.ts', '.js'],
  ...
  devServer: {
    ...
    proxy: {
      '/items': {
        target: proxy ? proxy : 'http://localhost:8080',
        secure: false,
      },
    },
    writeToDisk: true,
